...
objFile = open("ToDoList.txt", "w")   # An object that represents a file, creates new file
...
objFileToDo = open("ToDoList.txt", "r")
for strData in objFileToDo:
...
# Step 3 - Show the current items in the table
    if (strChoice.strip() == '1'):
        if not lstTable:
            ...
            print()
            for dicRow in lstTable:  # loop through Table and print row data
...
# Step 4 - Add a new item to the list/Table
        ...
        strPriority = input("Enter a priority level (1 to 5): ")
        print("You entered: " + strTask + " as priority level " + strPriority)
        dicRow = {"Task": strTask, "Priority": strPriority} # adds the input to the dictionary
        lstTable.append(dicRow) # adds row to table
        continue

# Step 5 - Remove a new item from the list/Table
    elif (strChoice.strip() == '3'):
        strTask = input("Which item would you like to remove? (Type BACK to return to menu):  ")
        for row in lstTable:
            if row["Task"] == strTask:
                lstTable.remove(row)
                print("Item deleted")
